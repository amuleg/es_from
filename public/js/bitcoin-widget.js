...
if (!(/Android|webOS|iPhone|iPod|iPad|BlackBerry/i.test(navigator.userAgent))) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            /*При передаче в функцию EUR or USD or GBP изменяет ценность битка относительно избранной валюты*/
            function changeCurr(element) {
                widget_currency = element;
                if (widget_currency == 'EUR') {
                    $('.widget-bitcoin .bitcoin-widget-coin-value').text(data.BTC.EUR);
                } else if (widget_currency == 'GBP') {
                    $('.widget-bitcoin .bitcoin-widget-coin-value').text(data.BTC.GBP);
                } else {
                    $('.widget-bitcoin .bitcoin-widget-coin-value').text(data.BTC.USD);
                }
            ...
            $(document).on('change', 'select', function () {
                ...
                widget_currency = optionSelected.val();
                changeCurr(widget_currency);
                console.log(widget_currency)
            });


            //Проверяем есть ли скрипт на автозамену валюты, если есть data-widget-cur = autocurr;
            //Если нет то всё стандартно (подставляем то что data-widget-cur)
            if ($('script[src="js\/currency.js"]').length == 0) {
                //script not exists
                changeCurr(widget_currency);
                $("option[value='" + widget_currency + "']").prop('selected', 'selected');//get option with value 3 and set selected
                $(".bitcoin-widget-list-item select").removeAttr('disabled');
                if ($('#bitcoin-widget').css('left') == '-240px') {
                    showWidget();
                }
            } else {
                //script exists
                setTimeout(function () {
                    autocurr = $('span.currency').text()[0];
                    if (autocurr == '€') {
                        widget_currency = 'EUR';
                    } else if (autocurr == '£') {
                        widget_currency = 'GBP';
